...
macro(_pivot_setup_boost)
    ...
    # elbo-sdk is loaded inside Blender (often Flatpak), which may not ship Boost shared libs.
    # Ensure Boost.Filesystem/System are available and linkable as static libs.
    if(NOT DEFINED Boost_USE_STATIC_LIBS)
        set(Boost_USE_STATIC_LIBS ON CACHE BOOL "Prefer static Boost libraries when available")
    endif()
    if(NOT DEFINED Boost_USE_MULTITHREADED)
        set(Boost_USE_MULTITHREADED ON CACHE BOOL "Link against multithreaded Boost libraries")
    endif()

    # Prefer system Boost static libs when available.
    find_package(Boost 1.75 QUIET COMPONENTS interprocess filesystem system)
    if(Boost_FOUND)
        if(NOT WIN32)
            foreach(_pivot_boost_lib_var IN ITEMS Boost_FILESYSTEM_LIBRARY Boost_SYSTEM_LIBRARY)
                if(DEFINED ${_pivot_boost_lib_var} AND NOT "${${_pivot_boost_lib_var}}" STREQUAL "")
                    if(NOT "${${_pivot_boost_lib_var}}" MATCHES "\\.a$")
                        message(FATAL_ERROR "${_pivot_boost_lib_var} is not a static library: ${${_pivot_boost_lib_var}}. Install/build Boost static libs so the wheel works in Blender.")
                    endif()
                endif()
            endforeach()
        endif()
        ...
        message(STATUS "System Boost filesystem/system not found; fetching Boost via FetchContent and building static libs")
        ...
        # Build static Boost.System (required by Boost.Filesystem)
        if(NOT TARGET Boost::system)
            file(GLOB BOOST_SYSTEM_SRCS "${boost_ext_SOURCE_DIR}/libs/system/src/*.cpp")
            if(NOT BOOST_SYSTEM_SRCS)
                message(FATAL_ERROR "Failed to locate Boost.System sources under ${boost_ext_SOURCE_DIR}")
            endif()
            add_library(boost_system_static STATIC ${BOOST_SYSTEM_SRCS})
            target_include_directories(boost_system_static PUBLIC ${boost_ext_SOURCE_DIR})
            # Boost 1.83+ may use std::atomic_ref in some libs; compile with C++20.
        ...
        # Build static Boost.Filesystem
        if(NOT TARGET Boost::filesystem)
            file(GLOB BOOST_FILESYSTEM_SRCS "${boost_ext_SOURCE_DIR}/libs/filesystem/src/*.cpp")
            if(NOT BOOST_FILESYSTEM_SRCS)
                message(FATAL_ERROR "Failed to locate Boost.Filesystem sources under ${boost_ext_SOURCE_DIR}")
            endif()
            add_library(boost_filesystem_static STATIC ${BOOST_FILESYSTEM_SRCS})
            target_include_directories(boost_filesystem_static PUBLIC ${boost_ext_SOURCE_DIR})
    ...
    # Boost::json: use system if available, otherwise build objects from fetched sources.
    if(NOT TARGET Boost::json)
        find_package(Boost 1.75 QUIET COMPONENTS json)
    endif()
    if(NOT TARGET Boost::json)
        if(NOT DEFINED boost_ext_SOURCE_DIR)
            include(FetchContent)
            set(BOOST_VERSION 1.83.0 CACHE STRING "Boost release version to fetch")
            string(REPLACE "." "_" BOOST_VERSION_U ${BOOST_VERSION})
            FetchContent_Declare(
